...
import { CheckCircle, RotateCcw, PlayCircle, Copy, Check } from 'lucide-react';
import VariableAutocomplete from './VariableAutocomplete';
import MarkdownResult from './MarkdownResult';

export default function ProofreaderNode({ id, data, selected }) {
  ...
  return (
    <div className={`bg-white rounded-xl border-2 shadow-lg w-[320px] transition-all ${
      selected ? 'border-purple-500 ring-4 ring-purple-200' : 'border-purple-300'
    }`}>
      <Handle 
        type="target" 
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold">{data?.name || 'Proofreader'}</span>
        </div>
        ...
        <div className="px-4 pb-4">
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-purple-700">Result:</p>
              <button
                onClick={handleCopy}
            ...
            <div className="text-xs text-purple-900 max-h-48 overflow-y-auto">
