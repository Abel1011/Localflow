...
import { RefreshCw, RotateCcw, PlayCircle, Copy, Check } from 'lucide-react';
import VariableAutocomplete from './VariableAutocomplete';
import MarkdownResult from './MarkdownResult';

export default function RewriterNode({ id, data, selected }) {
  ...
  return (
    <div className={`bg-white rounded-xl border-2 shadow-lg w-[320px] transition-all ${
      selected ? 'border-teal-500 ring-4 ring-teal-200' : 'border-teal-300'
    }`}>
      <Handle 
        type="target" 
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold">{data?.name || 'Rewriter'}</span>
        </div>
      ...
      <div className="p-4">
        <VariableAutocomplete
          ...
          onChange={(value) => data.onChange?.(id, 'instructions', value)}
          placeholder="Rewriting instructions..."
          rows={3}
          availableVariables={data.availableVariables || []}
          className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none"
        />
      </div>
      {data?.executionError && (
        ...
        <div className="px-4 pb-4">
          <div className="bg-teal-50 border border-teal-200 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-teal-700">Result:</p>
              <button
                onClick={handleCopy}
            ...
            <div className="text-xs text-teal-900 max-h-48 overflow-y-auto">
